...
{        
    ...
    public function showCopyBuntton($url_copy) {
    	...
    	$copyButton = '&nbsp;<a href="#" style="width:100px;background: #cfbaba;padding: 5px;border-radius: 5px;" class="btn-action" onClick="copyRecord(\''.$url_copy.'\')">'
		  				...
		  				if($i==0) {
    						...
    						if($link!=null){
    							foreach($link as $column=>$url)
    								if($key==$column){
    									$img='';
    									$array=array('tag'=>'a','attribute'=>array('href'=>Application_Form_FrmMessage::redirectorview($url).'/id/'.$temp,'target'=>"_blank"));
    									$read=$this->formSubElement($array,$img.$read);
    								}
    						}
    						$text='';
    						...
    						$row_str.='<td class="'.$class.'" '.$text.'>'.$read.'</td>';
			  				if($i == count($columns)) {
	    						if($editLink != "") {
									$row_str.='<td class="'.$class.'"><a class="edit" target="_blank" href="'.$editLink.'/id/'.$temp.'">'.'</a></td>';
			    				}
	    					}
    					}
    					$i++;
