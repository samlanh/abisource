...
//     	echo $sql;exit();
    ...
    public function getAllStudentCode(){
    	$db = $this->getAdapter();
    	$sql="SELECT stu_id as id,stu_code as code  FROM rms_student where status=1 and is_subspend=0  ORDER BY  id DESC ";
    	return $db->fetchAll($sql);
    ...
    public function getAllService(){
    	...
    	$_db = new Application_Model_DbTable_DbGlobal();
    	$branch_id = $_db->getAccessPermission("(branch=50 OR branch ",1);
    	
    	$sql="SELECT service_id as id , title as name  FROM rms_program_name WHERE type=2 and status=1 and ser_cate_id=3  $branch_id ";
    	return $db->fetchAll($sql);
    }
