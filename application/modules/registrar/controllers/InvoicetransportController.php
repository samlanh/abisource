<?php
class Registrar_InvoicetransportController extends Zend_Controller_Action {
	protected $tr;
    ...
    {
    	try{
    		$db = new Registrar_Model_DbTable_DbInvoiceTransport();
    		...
    		$rs_rows= $db->getAllInvoiceTransport($search);
    		
    		$list = new Application_Form_Frmtable();
    		$collumns = array("STUDENT_ID","NAME","SEX","INVOICE_NO","SERVICE","SUBTOTAL","CREATE_DATE","USER","STATUS");
    		$link=array(
    				'module'=>'registrar','controller'=>'invoicetransport','action'=>'edit',
    		);
    ...
    public function addAction()
    {
    	if($this->getRequest()->isPost()){
      	$_data = $this->getRequest()->getPost();
	      	try {
	      		$db = new Registrar_Model_DbTable_DbInvoiceTransport();
	      		$db->addInvoiceTransport($_data);
	      		if(isset($_data['save_new'])){
	      			Application_Form_FrmMessage::message($this->tr->translate('INSERT_SUCCESS'));
	      		}else{
	      			Application_Form_FrmMessage::Sucessfull($this->tr->translate('INSERT_SUCCESS'), self::REDIRECT_URL . '/invoicetransport/index');
	      		}
	      	} catch (Exception $e) {
       ...
       $frm = new Registrar_Form_FrmInvoiceTransport();
       ...
       array_unshift($service, array ( 'id' => -2, 'name' => $this->tr->translate('ADD_NEW') ));
       array_unshift($service, array ( 'id' => -1, 'name' => $this->tr->translate('SELECT_SERVICE') ));
...
//        print_r($db->getAllNewStudentName());exit();
       ...
       $db_car = new Registrar_Model_DbTable_DbStudentServicePayment();
       $this->view->all_car = $db_car->getAllCar();
    ...
    public function editAction()
    {
    	...
    	if($this->getRequest()->isPost()){
    		...
    		try {
    			$db = new Registrar_Model_DbTable_DbInvoiceTransport();
    			$db->updateInvoiceTransport($_data,$id);
    				Application_Form_FrmMessage::Sucessfull($this->tr->translate('EDIT_SUCCESS'), self::REDIRECT_URL . '/invoicetransport/index');
    		} catch (Exception $e) {
