...
class Global_FeeController extends Zend_Controller_Action {
    ...
    public function indexAction()
    {
    	try{
    		...
    		$link=array(
    				'module'=>'global','controller'=>'fee','action'=>'edit',
    		);
    	...
    	if($this->getRequest()->isPost()){
    		...
    		try {
	    		...
	    		if(isset($_data['save_close'])){
	    			Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/global/fee");
	    		}else{
	    			Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/global/fee/add");
	    		}
    		}catch(Exception $e){
    ...
    public function editAction()
	{
		...
		if($this->getRequest()->isPost()){
			try {
				...
				if(!empty($rs))Application_Form_FrmMessage::Sucessfull("INSERT_SUCCESS","/global/fee");
			}catch(Exception $e){
				Application_Form_FrmMessage::message("INSERT_FAIL");
				Application_Model_DbTable_DbUserLog::writeMessageError($e->getMessage());
			}
		...
		if(empty($feeid)){
			Application_Form_FrmMessage::Sucessfull("NO_DATA","/global/fee");
		}
...
// 				print_r($rows);exit();
	...
	function copyAction(){
		...
		if(!empty($rows))foreach($rows as $payment_tran){
			...
			elseif($payment_tran['payment_term']==5){
				$rs_rows[$key_old]['section'] = $payment_tran['tuition_fee'];
			}
		}
		$this->view->rows =$rs_rows;
