...
class Foundation_Model_DbTable_DbStudentDropTransport extends Zend_Db_Table_Abstract
{
	...
	public function addStudentDropTransport($_data){
		try{	
			...
			$sql="select id from rms_student_payment where student_id = ".$_data['studentid']." and payfor_type=3 and is_void=0 order by id DESC limit 1";
	...
	function getAllTransportService(){
		$db = $this->getAdapter();
		$sql = "select service_id as id,title as name from rms_program_name where type=2 and ser_cate_id = 3 ";
