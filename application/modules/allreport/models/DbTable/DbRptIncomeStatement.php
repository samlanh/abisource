...
class Allreport_Model_DbTable_DbRptIncomeStatement extends Zend_Db_Table_Abstract
{
...
//     	echo $sql.$where;
    ...
    function getAllExpense($search){
    	...
    	if($search['shift']==0){
	    	...
	    	$from_date =(empty($search['start_date']))? '1': "for_date >= '".$search['start_date']." 00:00:00'";
	    	$to_date = (empty($search['end_date']))? '1': "for_date <= '".$search['end_date']." 10:30:00'";
    	}
    	else if($search['shift']==2){
	    	$from_date =(empty($search['start_date']))? '1': "for_date >= '".$search['start_date']." 10:30:01'";
	    	$to_date = (empty($search['end_date']))? '1': "for_date <= '".$search['end_date']." 16:30:00'";
    	}
    	else if($search['shift']==3){
	    	$from_date =(empty($search['start_date']))? '1': "for_date >= '".$search['start_date']." 16:30:01'";
	    	$to_date = (empty($search['end_date']))? '1': "for_date <= '".$search['end_date']." 23:59:59'";
	    }
