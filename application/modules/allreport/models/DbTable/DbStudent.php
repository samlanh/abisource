...
class Allreport_Model_DbTable_DbStudent extends Zend_Db_Table_Abstract
{
	public function getAllStudentre($search){
		...
		$sql = "SELECT 
			...
			,stu_code,dob,remark,tel,(SELECT province_kh_name FROM `rms_province` WHERE `province_id`= rms_student.province_id limit 1) as pro,
			...
			(SELECT `kh_name` FROM `rms_dept` WHERE `dept_id`= degree limit 1) as degree,
			(SELECT `major_enname` FROM `rms_major` WHERE `major_id`=grade limit 1) as grade,
			(SELECT `name_kh` FROM `rms_view` WHERE TYPE=4 AND key_code =session limit 1)as session ,
			(SELECT (SELECT `title` FROM`rms_program_name` WHERE `service_id` = `level` limit 1) FROM `rms_study_history` WHERE `stu_id` = rms_student.stu_id limit 1) as level
