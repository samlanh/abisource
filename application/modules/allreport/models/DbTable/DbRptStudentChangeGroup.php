...
//     }
    public function getAllStudentChangeGroup($search){
    	...
    	$sql = "
	    	SELECT 
		    	stu_id,(SELECT CONCAT(stu_khname,' - ',stu_enname) FROM `rms_student` WHERE `rms_student`.`stu_id`=`rms_student_change_group`.`stu_id` limit 1) AS name,
				...
				(select name_kh from `rms_view` where `rms_view`.`type`=6 and `rms_view`.`key_code`=`rms_student_change_group`.`status` LIMIT 1)AS status
