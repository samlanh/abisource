...
class Allreport_Model_DbTable_DbRptStudentAttendent extends Zend_Db_Table_Abstract
{
   ...
   function getParentName(){
   		$db=$this->getAdapter();
   		$sql="SELECT 
   				(select subject_titleen from rms_subject where rms_subject.id=rms_score.parent_id LIMIT 1) As parent_id 
   			  FROM rms_score,rms_score_detail as sd 
   ...
   function getSubjectdByParent(){
   	...
   	$sql="SELECT 
   				...
   				(SELECT subject_titleen FROM rms_subject WHERE rms_subject.id=rms_score.subject_id LIMIT 1) AS subject_name 
   ...
   function getStudentAttendent(){
   	 	...
   	 	$sql="SELECT
				  ...
				  a.subject_id,
				  a.date,
				  (select subject_titleen from rms_subject where rms_subject.id=a.subject_id LIMIT 1) as subject_name,
				  ad.student_code,
				  ad.student_id,
				  ad.sex,
				  ad.grade_id,
