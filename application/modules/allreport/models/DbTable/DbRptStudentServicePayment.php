...
//     }
    function getAllBalance($search){
    	$db=$this->getAdapter();
    	$sql = "SELECT 
	    			spd.id,spd.fee AS fee,spd.discount_fix AS discount,spd.subtotal AS payment,spd.paidamount AS paid,spd.is_complete AS complete,
				...
				FROM rms_student_payment AS sp,rms_student_paymentdetail AS spd 
				WHERE spd.payment_id=sp.id and spd.balance>0
    		   ";
...
//     	$order=" ORDER BY id DESC ";
    	...
    	if($search['searchby']==1){
    		$where.=" AND (select CONCAT(from_academic,to_academic) from rms_servicefee where rms_servicefee.id=sp.year LIMIT 1) LIKE '%".$searchs."%'";
    	}
    	if($search['searchby']==2){
    		$where.=" AND (select stu_code from rms_student where rms_student.stu_id=sp.student_id LIMIT 1) LIKE '%".$searchs."%'";
    	}
    	if($search['searchby']==3){
    		$where.=" AND (select CONCAT(stu_khname,' - ',stu_enname) from rms_student where rms_student.stu_id=sp.student_id LIMIT 1) LIKE '%".$searchs."%'";
    	}
    ...
    function getFeebyOther($fee_id){
    	$db = $this->getAdapter();
    	$sql = "select *,
    	(SELECT major_enname FROM `rms_major` WHERE major_id=rms_tuitionfee_detail.class_id LIMIT 1) as class
