...
class Accounting_Model_DbTable_DbStudentLunchPayment extends Zend_Db_Table_Abstract
{
...
// 		}
	...
	function getAllOldStudent($branch_id){
    	...
    	where sv.stu_id = s.stu_id and sv.type=5 and sv.status=1 and sv.is_suspend=0 and sv.branch_id = $branch_id ";
    ...
    function getAllOldLunchId($student_id=0){
    	$db=$this->getAdapter();
    ...
    function deleteRecord($data,$id){
    	...
    	if(!empty($data['delete_student'])){
    		...
    		if(!empty($student_id)){
    			$where = " type=5 and stu_id = $student_id";
    			$this->_name = "rms_service";
    			$this->delete($where);
    		}
    	}
