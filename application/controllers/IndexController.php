...
// 					}	
    ...
    protected function sortMenu($menus){
    	$menus_order = Array ( 'home','registrar','accounting','allreport','foundation','global','rsvacl','setting');
        ...
        if ($this->getRequest()->isPost()){ 
    		...
    		if ($pass_data['password'] == $session_user->pwd){
				...
				try {
					...
					$session_user->lock();
					Application_Form_FrmMessage::Sucessfull('ការផ្លាស់ប្តូរដោយជោគជ័យ', '/home');
				} catch (Exception $e) {
